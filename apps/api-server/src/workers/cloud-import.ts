...
import { persistUploadedFile } from "../modules/sources/media";
import { queueJob } from "./job-queue";
import { Worker } from "bullmq";
import { redis } from "../infrastructure/redis";
import { CLOUD_IMPORT_QUEUE, cloudImportQueue } from "../infrastructure/queues";
...
async function processQueueItem(itemId: number) {
  try {
    ...
    if (content) {
      try {
        const { chunkText } = await import("../infrastructure/rag");
        const { generateEmbedding } = await import("../infrastructure/ai");
        const { pgvectorEnabled } = await import("../infrastructure/embed-init");
